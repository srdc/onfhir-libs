...
object ImMemorySearchUtil {
   ...
   * @return
   */
  def handleSimpleParameter(parameter:Parameter, sp:SearchParameterConf, values:Seq[(Seq[JValue], String)], endpointSettings: FhirEndpointSettings): Boolean ={
    parameter.suffix match {
      ...
      case _ =>
        handleParameter(parameter, sp, values, endpointSettings)
    }
  }
   ...
   * @return
   */
  def handleCompositeParameter(parameter:Parameter, sp:SearchParameterConf, values:Seq[(Seq[JValue], String)], validSearchParamConfs:Map[String, SearchParameterConf], endpointSettings: FhirEndpointSettings):Boolean = {
   ...
   * @return
   */
  private def handleParameter(parameter:Parameter, sp:SearchParameterConf, values:Seq[(Seq[JValue], String)], endpointSettings: FhirEndpointSettings):Boolean = {
    parameter.valuePrefixList.exists {
      case (prefix, value) =>
        ...
        if (parameter.paramType == FHIR_PARAMETER_TYPES.REFERENCE)
          referenceQueryOnValues(value, prefixModifier, values, sp.targets, endpointSettings)
        else
    ...
    valuesAndTargetType.exists(valuesAndTarget =>
      valuesAndTarget._1.exists(value => {
        valuesAndTarget._2 match {
          ...
          case other => throw new IllegalStateException(s"Unknown target element type $other !!!")
        }
      })
    )
  }
   ...
   * @return
   */
  private def referenceQueryOnValues(reference:String, modifier:String, valuesAndTargetTypes:Seq[(Seq[JValue], String)], referenceTargetTypes:Seq[String], endpointSettings: FhirEndpointSettings):Boolean = {
                ...
                .exists(
                  _.extractOpt[String]
                    ...
                    .exists { case (actualUrl, actualResourceType, actualRid, actualVersion) =>
                      actualResourceType == rtype &&
                        actualRid == rid &&
                        isUrlMatching(actualUrl, url, endpointSettings) &&
  ...
  /**
    ...
    * @return
    */
  private def isUrlMatching(actualUrl:Option[String], queryUrl:Option[String], endpointSettings: FhirEndpointSettings) = {
    (actualUrl, queryUrl) match {
      case (None, None) => true //if not url in both it is ok
      case (Some(endpointSettings.rootUrl), None) => true //if one of them is FHIR root URL and other is missing, it is OK
      case (None, Some(endpointSettings.rootUrl)) => true
