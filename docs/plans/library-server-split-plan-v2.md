...
| A FHIRPath `%name` that no caller supplied falling through to the OS environment, so `FhirPathEnvironment.getEnvironmentContext` returned the process environment value | the same variable resolves to empty and `sys.env` is never consulted. Deliberate security fix: expressions arrive from profile invariants, search parameter definitions, subscription criteria and mapping templates, so the fallthrough let content that the operator did not author read process secrets and surface them through validation diagnostics or a mapped resource. A deployment that relied on it must pass the value explicitly with `withEnvironmentVariable` | 5A - implemented 2026-08-06 |
