...
abstract class FHIRPaginatedBundle(bundle:Resource, val request:FhirRequestBuilder)  extends FHIRBundle(bundle) {
  ...
  def getLastPage():Long = links.get("last").flatMap(queryValue(_, "_page")).map(_.toLong).getOrElse(if(entries.isEmpty) 0L else 1L)
  ...
  def getNextPage(pgnParam: String): Option[String] =
    links.get("next").flatMap(queryValue(_, pgnParam))
  ...
  private def queryValue(uri: String, name: String): Option[String] =
    Option(URI.create(uri).getRawQuery)
      .map(OrderedQuery.parse)
      .flatMap(_.toMultiMap.get(name).flatMap(_.headOption))
}
...
class FHIRHistoryBundle(bundle:Resource, override val request:FhirHistoryRequestBuilder) extends FHIRPaginatedBundle(bundle, request) {
  ...
  case class BundleResponseEntry(status:String, lastUpdateTime:Instant, version:Option[Long])
   ...
   * @return
   */
  def getHistory():Seq[(Long, Option[Resource], Instant)] = historyEntries.head._2.map(e => (e._1 , e._4, e._3.lastUpdateTime))
   ...
   * @return
   */
  def getHistory(rid:String):Seq[(Long, Option[Resource], Instant)] =
    historyEntries
      .getOrElse(rid, Nil)
   ...
   * @return
   */
  def getHistories:Map[String, Seq[(Long, Option[Resource], Instant)]] =
    historyEntries
  ...
  private def parseBundleResponse(br:JObject): BundleResponseEntry = {
    BundleResponseEntry(
      ...
      DateTimeUtil.parseFhirDateTimeOrInstant(FHIRUtil.extractValue[String](br, "lastModified")),
...
class FHIRTransactionBatchBundle(val bundle:Resource) extends FHIRBundle(bundle) {
  ...
  private def parseEntryAsResponse(entry:JObject):FHIRResponse = {
    ...
    val statusCode = HttpStatus(FHIRUtil.extractValue[String](rsp, "status").trim.split(' ').head.toInt)
    ...
    new FHIRResponse(
      ...
      location = FHIRUtil.extractValueOption[String](rsp, "location").map(URI.create),
      lastModified = FHIRUtil.extractValueOption[String](rsp, "lastModified").map(DateTimeUtil.parseFhirDateTimeOrInstant),
