 ...
 * @param allSearchParameters URL of all search parameters supported by the resource type and their codes
 *                            e.g. http://hl7.org/fhir/SearchParameter/Patient-identifier -> identifier
 */
class SearchParameterConfigurator(
                                   rtype: String,
                                   rtypeBaseProfile: Option[String],
                                   fhirConfig: FhirServerConfig,
                                   allSearchParameters: Map[String, String]
      ...
      extensionPaths.map(ep => {
        ...
        val finalPath =
          finalMainPart match {
            ...
            case _ => finalMainPart + "." + parts.dropWhile(_ != "extension[i]").mkString(".")
          }
  ...
  private def transformPathsAndExtractTargetTypes(ptype: String, pathsAndRestrictions: Seq[(String, Seq[(String, String)])]): (Seq[String], Seq[String], Seq[Seq[(String, String)]], Seq[Set[String]]) = {
    ...
    val filteredPaths: Seq[(String, String, Seq[(String, String)], Set[String])] =
      pathsAndRestrictions.flatMap {
