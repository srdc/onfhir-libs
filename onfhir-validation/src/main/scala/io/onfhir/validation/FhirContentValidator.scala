...
class FhirContentValidator(
      ...
      .map(rr => {
        Future.sequence(
          referencesToCheck
            .map(reference =>
              rr
                .isReferencedResourceExist(reference._1, reference._2)  //Check if reference exists
                .map(isExist => reference -> isExist)
            )
        ).map(referencesExistence =>
   ...
   * @param parentPath                  Parent path for the validated content
   ...
   * @param forceRecognitionOfElementsEvenForAbstractChain  Force recognition of elements for abstract profiles
   * @return
   */
  def validateComplexContentAgainstProfile(
                                            profileChain: Seq[ProfileRestrictions],
                                            value: JObject,
                                            parentPath: Option[String],
                                            resourceElementRestrictions: Seq[Seq[(String, ElementRestrictions)]] = Nil,
                                            forceRecognitionOfElementsEvenForAbstractChain:Boolean = false): Future[Seq[OutcomeIssue]] = {
    logger.debug(s"Validating against profile ${profileChain.head.url}...")
    if(profileChain.map(_.url).contains(s"$FHIR_ROOT_URL_FOR_DEFINITIONS/StructureDefinition/Bundle"))
      bundle = Some(value)
    ...
    val issuesFuture =
      Future.sequence(
        value.obj
          .filter(!_._1.startsWith("_"))  // Only get the normal elements (filter out the extensions on primitives)
          .map {
            ...
            case (field, fieldValue) =>
              ...
              extractFieldNameAndDataType(field, allRestrictions) match {
                ...
                case None =>
                  Future.apply {
                    if (!isAllAbstract || resourceElementRestrictions.exists(_.nonEmpty) || forceRecognitionOfElementsEvenForAbstractChain)
                  ...
                  if(cardinalityIssues.nonEmpty)
                    ...
                    fieldValue match {
                      case JArray(arr) =>
                        Future
                          .sequence(
                            arr.map(i => validatePrimitiveExtension(FHIRUtil.mergeElementPath(parentPath, s"_$field"), i))
                          )
                          .map(_.flatten)
                      case v:JValue =>
    ...
    for {
      issues <- issuesFuture
      primitiveWithoutValuesIssues <- primitiveWithoutValuesIssuesFuture
    } yield issues.flatten ++ primitiveWithoutValuesIssues.flatten ++ requiredConstraintIssues ++ fhirConstraintIssues
  }
          ...
          case JArray(arr) =>
            ...
            if(!possiblePrimitiveExtension.forall( pp => pp.isInstanceOf[JArray] && pp.asInstanceOf[JArray].arr.length == arr.length))
              ...
              Future
                .sequence(
                  arr.zipWithIndex
                    .map(v => furtherValidateElement(path + s"[${v._2}]", fieldName, dataType, v._1, normalDefinitions,
   ...
   * @return
   */
  private def furtherValidateSlicedElement(allSlicingDefs: Seq[FhirSlicing], path: String, fieldName: String, dataType: String, value: JValue, elementRestrictions: Seq[(Option[ElementRestrictions], Seq[(String, ElementRestrictions)])]): Future[Seq[OutcomeIssue]] = {
          ...
          sliceValueMatchings._1 match {
            ...
            case Some(svm) =>
                ...
                Future
                  .sequence(
                    sliceValueMatchings._2
                      .map(v => furtherValidateElement(path + s"[${v._2}]", fieldName, dataType, v._1, (Some(svm._4) -> svm._5.flatten) +: normalDefinitions))
                  ).map(errorsOnSlice =>
                      errorsOnSlice
                        .flatten
                        .map(e => e.copy(diagnostics = Some(s"Based on the slice definition '${svm._1}': ${e.diagnostics.get}")))
                  )
          }
        } else
          Future.apply(FhirContentValidator.convertToOutcomeIssue(path, sliceErrors))
      })

    Future
      .sequence(slicingErrors)
      .map(_.flatten)
  }
   ...
   * @param ppe   Extension content
   * @return
   */
  private def validatePrimitiveExtension(path:String, ppe:JValue):Future[Seq[OutcomeIssue]] = {
    if(!ppe.isInstanceOf[JObject])
      Future.apply(FhirContentValidator.convertToOutcomeIssue(path, Seq(ConstraintFailure(s"Extension for FHIR primitive element within array should be given as JSON object!"))))
    else
   ...
   * @return
   */
  private def furtherValidateElement(path: String, fieldName: String, dataType: String, value: JValue, elementRestrictionChain: Seq[(Option[ElementRestrictions], Seq[(String, ElementRestrictions)])], possiblePrimitiveExtension:Option[JValue] = None): Future[Seq[OutcomeIssue]] = {
    ...
    if (allWarningsOrErrorsForThis.nonEmpty && allWarningsOrErrorsForThis.exists(!_.isWarning))
      Future.apply(FhirContentValidator.convertToOutcomeIssue(path, allWarningsOrErrorsForThis))
    //Otherwise, and if it is complex continue with childs
      ...
      if(possiblePrimitiveExtension.isDefined) {
        ...
        Future.apply(FhirContentValidator.convertToOutcomeIssue(actualPath, Seq(ConstraintFailure(s"Unrecognized element $actualPath !"))))
      } else {
        ...
        val profileChains =
          profileUrls
            .map(url => fhirConfig.findProfileChainByCanonical(url)).sortWith((c1, c2) => c1.size >= c2.size)
        //Get all urls
      ...
      possiblePrimitiveExtension
        .map(ppe => validatePrimitiveExtension(path, ppe))
        .getOrElse(Future.apply(Nil))
        .map(extensionErrors =>
          extensionErrors ++
    ...
    elementRestrictions.flatMap(e =>
      e._1.flatMap(_.slicing) match {
        case None if e._2.map(_._1).contains(fieldName + ":") =>
          ...
          Some(FhirSlicing(Seq("type" -> "$this"), ordered = false, "open"))
        case oth => oth
      }
    )
  }
  ...
  private def findPossibleElementsForMatchers(discriminatorType:String, discriminatorPath:String, sliceRestriction: ElementRestrictions, sliceSubElements: Seq[Seq[(String, ElementRestrictions)]]): Either[Seq[Seq[(String, ElementRestrictions)]], Seq[ProfileRestrictions]]  = {
    ...
    if(discPathItems.contains("resolve()")){
      ...
      afterResolvingPathItems match {
        ...
        case Nil =>
          Right(fhirConfig.findProfileChainByCanonical(referencedProfile))
        case _ =>
          //Otherwise Load the profile chain, and find the definition path for it
          val subElementsOfReferenced = fhirConfig.findProfileChainByCanonical(referencedProfile).map(_.elementRestrictions)
  ...
  private def findRequiredElements(allElementRestrictions: Seq[Seq[(String, ElementRestrictions)]]): Map[String, Seq[String]] = {
    val requiredElementPaths: Set[String] =
      allElementRestrictions
        .map(er => er.filter(e => !e._1.contains('.'))) //Only get the children not descendants
