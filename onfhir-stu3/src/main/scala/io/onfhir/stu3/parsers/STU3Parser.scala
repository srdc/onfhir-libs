...
/**
 * Parser for FHIR foundation resources for FHIR STU3.
 *
 * It extends [[R4Parser]] and overrides only the foundation-resource fields
 * whose STU3 shape differs from R4:
 *
 *  - `CapabilityStatement.rest.resource.profile` is a Reference in STU3, so the
 *    profile URL is read from `profile.reference`;
 *  - `CapabilityStatement.rest.compartment` sits under `rest` rather than the
 *    server `rest` entry that R4 uses;
 *  - `SearchParameter` has no `multipleOr`/`multipleAnd`, and its `component`
 *    order is significant;
 *  - `OperationDefinition.parameter.binding` uses `valueSetUri` or
 *    `valueSetReference.reference` instead of R4's canonical `valueSet`;
 *  - `ElementDefinition.type.profile`/`targetProfile` are single values, handled
 *    by [[STU3StructureDefinitionParser]].
 *
 * @param complexTypes       List of FHIR complex types defined in the standard
 * @param primitiveTypes     List of FHIR simple types defined in the standard
 * @param capabilityDefaults Defaults for omitted CapabilityStatement resource fields
 */
class STU3Parser(complexTypes: Set[String],
                 primitiveTypes: Set[String],
                 capabilityDefaults: FhirCapabilityDefaults = FhirCapabilityDefaults.Standard)
  extends R4Parser(complexTypes, primitiveTypes, capabilityDefaults) {
